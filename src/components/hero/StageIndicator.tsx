...
export const StageIndicator: React.FC<StageIndicatorProps> = ({ currentStage, progress }) => {
  const stages = [
    ...
    { num: 2, label: '02. Holographic Scan' },
    { num: 3, label: '03. Laser Surgery' },
    { num: 4, label: '04. Restored Teeth' },
    { num: 5, label: '05. Perfect Smile' },
  ];
  ...
  return (
    <div className="fixed right-6 top-1/2 -translate-y-1/2 z-30 hidden xl:flex flex-col items-end gap-5 pointer-events-auto">
      {stages.map((st) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={`text-[11px] font-mono font-medium tracking-wider transition-all duration-300 ${
                isActive
                  ? 'text-[#4FC3F7] opacity-100 translate-x-0'
                  : 'text-slate-500 opacity-0 group-hover:opacity-100 group-hover:text-slate-300 translate-x-2'
              }`}
            >
              {st.label}
            </span>
            ...
            <div className="relative flex items-center justify-center w-6 h-6">
              ...
              <div
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  isActive
                    ? 'w-3.5 h-3.5 bg-[#00E5FF] shadow-[0_0_12px_#00E5FF]'
                    : 'bg-slate-700 group-hover:bg-[#4FC3F7]/60'
                }`}
              />
            </div>
          </button>
      ...
      {/* Progress Track Line */}
      <div className="absolute right-[11px] top-3 bottom-3 -z-10 w-[2px] bg-slate-800">
        <div
          className="w-full bg-[#4FC3F7] transition-all duration-200 shadow-[0_0_8px_#4FC3F7]"
